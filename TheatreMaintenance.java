...
public class TheatreMaintenance {
	...
	public static void main(String[] args) {
		...
		System.out.println("Welcome to J12 Theartre Maintenance System");
		...
		if (ans.equalsIgnoreCase("yes") || ans.equalsIgnoreCase("y")) {
			...
			case "b":
				...
				//System.out.println("The available seats are " + screen.getTotalAvailableSeats(cust.getShowtime()));
				t = s.issueTicket(bookers.remove(0), cust.getChoiceOfSeats(), cust.getShowtime());
				if(null!=t) {
				System.out.println("The available seats are now " + (s.getTotalAvailableSeats(cust.getShowtime())));
				}
				if (null != t) {
					TicketPrint.FileWriter(t);
				} else {
					System.out.println("Ticket could not be printed as you entered the same booked seat, try again");
				}
				break;
